...
## `pelagos-cri::cni::tests::test_run_bounded_limits_concurrency`
**Does not require root** (`pelagos-cri` unit test, `cargo test -p pelagos-cri --bin pelagos-cri`). Spawns `MAX_CONCURRENT_CNI_CALLS * 3` (24) concurrent calls through `run_bounded` — the semaphore-gated wrapper shared by `cni_add_bounded`/`cni_del_bounded` — each closure recording how many are simultaneously active (via an `AtomicUsize` high-water mark) before sleeping 30ms. Asserts the observed peak concurrency never exceeds `MAX_CONCURRENT_CNI_CALLS` (8). **Why it matters:** after a node reboot, kubelet can reconcile dozens of pods at once, each triggering a CNI ADD; if the CNI provider (e.g. Cilium) isn't up yet, every plugin invocation blocks retrying its own agent socket connection. Without a cap, that burst thundering-herds the CNI provider itself, competing with — and slowing down — the very pod everyone is waiting on. This test is the regression guard for the semaphore bound (#494).
