...
## `issue_472_watcher_orphan::test_pdeathsig_kills_container_when_parent_dies`
**Requires root** (spawns a real container via `pelagos::container::Command`). Forks a child ("the watcher") that spawns a long-running container (`sleep 60`) via `pelagos::Command` and immediately exits. Verifies that the container process dies within 2 seconds of the watcher's exit. The reliability guarantee comes from Rust's `spawn()` blocking on the pre_exec error pipe until the grandchild has exec'd — so `PR_SET_PDEATHSIG` is installed before the watcher ever exits. **Why it matters:** without `PR_SET_PDEATHSIG` in the pre_exec hook, a watcher that dies (e.g. during a pelagos upgrade) leaves its container running as an orphan reparented to PID 1. The orphan holds bound ports, and all subsequent restart attempts for the same pod fail with `bind: address already in use`, putting the pod into permanent CrashLoopBackOff (#472).

## `issue_472_watcher_orphan::test_kill_orphaned_containers_startup_scan`
**Requires root** (writes to `/run/pelagos/containers/` and sends signals). Spawns a real `sleep 60` process to act as an orphaned container, writes a synthetic `state.json` with `status=running`, a guaranteed-dead `watcher_pid`, and the orphan's live PID, then starts `pelagos-cri` briefly on a temp socket and waits 500 ms. Asserts that the orphan process was killed (non-zero exit) by the time `wait()` returns. **Why it matters:** `PR_SET_PDEATHSIG` prevents NEW orphans, but containers orphaned before the fix (or before a `pelagos-cri` restart) would linger indefinitely. The startup scan is the recovery path: it detects and kills existing orphans before the CRI accepts any kubelet requests, clearing stuck CrashLoopBackOff conditions (#472).

## `issue_474_stale_sandbox_kills_containers::test_stale_sandbox_kills_running_container`
**Requires root** (writes to `/run/pelagos/containers/` and `/run/pelagos-cri/`, sends signals). Spawns a real `sleep 60` process to represent a Running container. Creates a fake CRI sandbox state with a guaranteed-dead `pause_pid` (triggering stale-sandbox detection) and a fake CRI container state in `Running` status that references the real process via `pelagos_name`. Writes a minimal pelagos `state.json` so `pelagos stop --time 0` can reach the process. Starts `pelagos-cri` and waits 2 s for the startup stale-sandbox reap to run. Asserts that the sleep process is dead. **Why it matters:** when systemd stops `pelagos-cri.service` (e.g. after a glibc upgrade by `needrestart`), all `pelagos-sbx-*.service` pause processes are stopped as dependents, but watcher and container processes survive. On restart the stale-sandbox reap detected the dead pauses and removed their CRI metadata — but did not kill the surviving container processes. Those processes held host port bindings (MetalLB speaker on port 7946), BoltDB file locks (Vault with Raft storage), and Unix sockets, causing every subsequent pod restart to fail with EADDRINUSE or bolt timeout. The fix makes `reap_stale_sandboxes` return the names of Running containers in reaped sandboxes so callers can issue `pelagos stop --time 0` for each, applied unconditionally for all container types (not just hostNetwork) (#474).

## `issue_477_no_duplicate_inherited_mounts::test_no_duplicate_mountinfo_on_inherited_mountpoint`
**Requires root** (spawns a real container with a MOUNT namespace). Binds a host tmpdir onto `/sys/fs/bpf` inside the container (a path that typically has an inherited `bpffs` mountpoint in the new MOUNT namespace before our fix). Reads `/proc/self/mountinfo` from inside the container and counts entries whose mount-point field equals `/sys/fs/bpf`. Asserts exactly one entry. **Why it matters:** when a new mount namespace is created, it inherits the host's mount table, including any submounts under `/sys` (e.g. `bpffs` at `/sys/fs/bpf`). A plain `mount(src, tgt, MS_BIND|MS_REC)` onto an already-mounted path stacks a second entry on top — both remain visible in `mountinfo`. Cilium's BPF map subsystem iterates `mountinfo` to locate the `bpffs` mount and aborts when it finds two entries, blocking network policy enforcement for all pods. The fix calls `umount2(tgt, MNT_DETACH)` before each hostPath bind, displacing any inherited mount first so the final count is always exactly one (#477).

## `issue_492_no_duplicate_mountinfo_bidirectional::test_no_duplicate_mountinfo_bidirectional_bpf`
**Requires root** (spawns a real container with a MOUNT namespace and an overlay rootfs). Skips automatically if the host's `/sys/fs/bpf` is not mounted with shared propagation (`shared:N` in `/proc/self/mountinfo`), since the bug only manifests when the source is a shared-peer-group mount. Uses `with_bind_mount_propagated(Bidirectional)` on `/sys/fs/bpf` with an overlay merged dir as effective root — the exact code path used by pelagos-cri for Cilium's `bpf-maps` hostPath volume. Reads `/proc/self/mountinfo` from inside the container and asserts exactly one entry at `/sys/fs/bpf`. **Why it matters:** for Bidirectional (rshared) bind mounts, pelagos calls `mount(null, src, MS_SHARED|MS_REC)` on the source before binding, converting the inherited slave mount to a shared peer (joining the host's bpffs peer group). This leaves the inherited source visible in the child namespace alongside the explicit bind. Depending on kernel version and peer-group propagation behaviour, both can appear as separate entries under `/sys/fs/bpf` in the container's `mountinfo` after `pivot_root`. Cilium's pre-flight check fails with "multiple mount points detected at /sys/fs/bpf" and crash-loops the entire DaemonSet. The fix calls `umount2(src, MNT_DETACH)` after the bind to remove the inherited source from the child namespace; the bind retains its peer-group membership independently so bidirectional propagation continues to work (#492).

## `issue_478_var_run_writable_with_readonly_rootfs::test_bind_mount_via_var_run_symlink_writable_with_readonly_rootfs`
**Requires root** (spawns a real container with MOUNT namespace and a readonly rootfs). Builds an isolated Alpine rootfs copy where `/var/run` is an absolute symlink to `/run` (matching Ubuntu OCI base images and Ubuntu 24.04+ hosts). Sets `readOnlyRootFilesystem: true`. Bind-mounts a host tmpdir onto `/var/run/test-478` (which resolves via the image's `/var/run → /run` symlink to `/run/test-478`). Verifies the container can write a file inside the bound path. **Why it matters:** on Ubuntu 24.04+ hosts, the Cilium DaemonSet requests a hostPath bind mount for `/var/run/cilium/envoy/sockets` (a path under the host's `/var/run → /run` symlink). Without canonicalization of the source path before `mount(2)`, certain kernel/filesystem combinations can fail to correctly associate the bind with the underlying tmpfs, causing EROFS when Envoy tries to create Unix sockets — breaking all network policy enforcement in Cilium-managed clusters (#478).

## `issue_484_bind_mount_writable_without_ro_rootfs::test_writable_bind_without_readonly_rootfs`
**Requires root** (spawns a real container with MOUNT namespace). Bind-mounts a writable host tmpdir onto `/bind-test-484` inside the container with `readOnly=false` and `readOnlyRootFilesystem` also unset (the default writable case). Writes a file into the mount from inside the container and reads it back. **Why it matters:** a bind mount established with `MS_BIND|MS_REC` inherits the source mount's kernel flags, including `MS_RDONLY` if the source filesystem was transiently read-only or if a kernel version quirk propagates the flag. Without an explicit `MS_REMOUNT|MS_BIND` (without `MS_RDONLY`) after the initial bind, the mount can appear read-only to the container even though the source directory is writable on the host and the volume spec says `readOnly: false`. Cilium's `cilium-envoy` DaemonSet hits this when binding `/var/run/cilium/envoy/sockets` and then trying to create Unix sockets — breaking all L7 NetworkPolicy (#484).

## `issue_484_bind_mount_writable_without_ro_rootfs::test_writable_bind_via_var_run_symlink_without_ro_rootfs`
**Requires root** (spawns a real container with MOUNT namespace). Same as the previous test but in a rootfs where `/var/run` is an absolute symlink to `/run` (matching Ubuntu 24.04+ OCI base images). Bind-mounts a host tmpdir onto `/var/run/test-484` (which resolves via the symlink to `/run/test-484`) without `readOnlyRootFilesystem`. Verifies the container can write a file into the bound path. **Why it matters:** the symlink in the container rootfs exercises the `resolve_mount_target_in_root` path, ensuring the combination of symlink resolution AND writable-remount both apply correctly for the `/var/run` → `/run` case that Cilium's envoy DaemonSet uses (#484).

## `issue_484_bind_mount_writable_without_ro_rootfs::test_overlapping_bind_mounts_child_before_parent`
**Requires root** (spawns a real container with MOUNT namespace and `readOnlyRootFilesystem`). Sets up a rootfs with `/var/run → /run` symlink (Ubuntu-like). Registers two bind mounts in the wrong order — child (`/var/run/cilium/envoy/sockets`) before parent (`/run/cilium`) — and verifies that the child mount is writable after the depth-sort fix is applied. **Why it matters:** without sorting bind mounts by destination path depth (ascending), the child bind is established first and the subsequent parent bind covers the intermediate directory with an empty source directory, making the child mount path unreachable via path resolution (the kernel walks through the empty parent source and finds no `envoy/sockets` subdirectory). On a read-only rootfs, the kernel falls back to the overlay directory which is EROFS. Sorting ensures the parent is always mounted before any of its children, matching the behavior of containerd/CRI-O (#484).

## `issue_484_bind_mount_writable_without_ro_rootfs::test_writable_bind_with_nested_mnt_rdonly`
**Requires root** (spawns a real container with MOUNT namespace). Sets up a parent directory containing a nested mount (a bind of a writable tmpfs, then remounted read-only via `MS_BIND|MS_REMOUNT|MS_RDONLY` — which sets `MNT_RDONLY` on the per-mount entry without touching the superblock's `SB_RDONLY`). Binds the parent into a container with `MS_BIND|MS_REC`; the nested bind copy inherits `MNT_RDONLY=true` from the source. Asserts the container can write inside the nested path. **Why it matters:** `MS_BIND|MS_REC` carrying a nested mount whose `MNT_RDONLY` flag is set (but `SB_RDONLY` is clear) is exactly how Cilium's prior container runs can leave `/run/cilium/envoy/sockets` read-only in a new container's namespace. `mount_setattr(AT_RECURSIVE, attr_clr=MOUNT_ATTR_RDONLY)` clears `MNT_RDONLY` recursively without needing to touch the superblock, making the path writable. Failure indicates the AT_RECURSIVE clear regressed (#484).

## `issue_483_env_var_propagation::test_env_var_visible_in_container`
**Requires root** (spawns a real container with MOUNT namespace). Spawns a shell container with `BIN_PATH=/var/lib/rancher/k3s/data/current/bin` set via `.env()` and verifies the variable is visible inside via `echo "BIN_PATH=$BIN_PATH"`. **Why it matters:** Cilium's `apply-sysctl-overwrites` init container expands `$BIN_PATH` in a `nsenter` command; if the variable is absent or contains unexpected bytes, bash word-splitting produces `/cilium-sysctlfix` (just the filename, no directory) which fails with ENOENT. This test is the library-level regression guard; the CRI-level fix (stripping trailing NUL bytes from proto `bytes` fields) is covered by the unit test below (#483).

## `issue_483_env_var_propagation::test_nul_byte_stripped_from_env_value`
**Does not require root** (pure logic test). Simulates the `pelagos-cri` decode path: takes a raw byte slice with a trailing NUL (`b"/path\x00"`) and asserts that filtering all NUL bytes produces a clean string. Also verifies mid-string NULs (e.g. `b"/path\x00with\x00nuls"`) are stripped — the old `strip_suffix(b"\0")` approach only removed a single trailing NUL, leaving any embedded NULs in place. **Why it matters:** the CRI proto `KeyValue.value` is a `bytes` field; some kubelet/proto library versions append a terminating NUL. Without stripping, `String::from_utf8_lossy` includes the NUL in the decoded value. When bash expands an env var containing a NUL character unquoted (e.g. `${BIN_PATH}/bin`), the NUL terminates the C-string and the remainder becomes a separate word, causing the shell to execute `/bin` (or similar) instead of the intended full path (#483).

(`test_run_bounded_limits_concurrency` was renamed to `test_run_bounded_limits_concurrency_and_fails_fast_over_capacity` in #500 — see the `pelagos-cri::cni` section below for its current entry.)

## `pelagos-cri::cri_metrics::tests::test_metrics_endpoint_serves_build_info`
**Does not require root** (`pelagos-cri` unit test, `cargo test -p pelagos-cri --bin pelagos-cri`). Binds an ephemeral loopback port, calls `cri_metrics::install()` (the same function `main.rs` calls at startup), then opens a raw `TcpStream` and issues a plain-text `GET /metrics HTTP/1.1` request (no HTTP client dependency needed for this). Asserts the response starts with `HTTP/1.1 200` and its body contains the `pelagos_cri_build_info{version="..."}` gauge with the crate's own version label. Deliberately a single test function, not several: `PrometheusBuilder::install()` registers a process-global recorder via `metrics::set_global_recorder` and panics if called a second time in the same test binary, so every assertion about the live endpoint has to happen within one `install()` call. **Why it matters:** this is the foundation sub-issue of #496 (Prometheus metrics epic) — before any per-RPC or per-CNI-call instrumentation can land, the `/metrics` HTTP endpoint itself has to actually serve valid, scrapeable Prometheus text format. A regression here (wrong content-type, endpoint not bound, recorder never installed) would silently break every metric added in later sub-issues without any of their own tests catching it, since they only assert metric *values* update, not that the endpoint is reachable at all (#497).

## `pelagos-cri::grpc_metrics::tests::test_grpc_method_label_hot_path_methods` / `test_grpc_method_label_unlisted_methods_collapse_to_other`
**Does not require root** (pure logic tests). Asserts `grpc_method_label()` maps known hot-path gRPC paths (e.g. `/runtime.v1.RuntimeService/RunPodSandbox`) to their method name, and that unlisted methods (e.g. `Version`), malformed paths, and the empty string all collapse to `"other"`. **Why it matters:** the whole point of a match statement instead of using the raw path segment as the label is a deliberately bounded cardinality — if a future edit accidentally changed the fallback to echo the raw path instead of `"other"`, an unexpected or malformed request path would leak an unbounded label straight into Prometheus (#498).

## `pelagos-cri::grpc_metrics::tests::test_in_flight_gauge_reflects_pending_request`
**Does not require root** (`pelagos-cri` unit test, `current_thread` runtime — same thread-local recorder reasoning as the other `grpc_metrics`/`cni` metrics tests). Wraps a `SlowEchoService` stand-in (whose response doesn't resolve until the test calls `release.notify_one()`, via `tokio::sync::Notify`) with `MetricsLayer`, spawns the call as a background task, yields twice so the task actually reaches its `.await` point, then takes exactly **one** snapshot and asserts `pelagos_cri_grpc_requests_in_flight` reads `1.0`. Deliberately only one snapshot per test — `metrics_util::debugging::DebuggingRecorder`'s `Snapshotter::snapshot()` reports gauges as the delta since the last snapshot (it swaps the stored value to 0 on read), so a second snapshot in the same test wouldn't mean what it looks like it means; see the companion test below instead for the completion-side assertion. **Why it matters:** this is the only test that proves the gauge reflects a request that is *genuinely still in flight*, not just an increment immediately followed by a decrement around a call that always resolves synchronously — which is the entire point of #501 (early warning before full saturation, independent of the per-method histograms in #498).

## `pelagos-cri::grpc_metrics::tests::test_in_flight_gauge_nets_to_zero_after_request_completes`
**Does not require root** (`pelagos-cri` unit test). Companion to the test above: runs a request through `MetricsLayer` to completion (fast, no artificial delay), then takes its own single first-ever snapshot and asserts the in-flight gauge nets to `0.0`. **Why it matters:** proves the increment (on request start) and decrement (`InFlightGuard`'s `Drop` impl, on completion) stay balanced — a missing decrement would show `+1.0` here, not `0.0`. The `Drop`-based guard (rather than a plain increment-then-decrement wrapped around the `.await`) specifically exists so a client disconnecting mid-request — which drops the future without running any code after its suspension point — still decrements the gauge instead of leaking a permanently-stuck `+1` (#501).

## `pelagos-cri::grpc_metrics::tests::test_metrics_layer_records_request_for_hot_path_method`
**Does not require root** (`pelagos-cri` unit test). Installs a `metrics_util::debugging::DebuggingRecorder` on a `current_thread` runtime (same reasoning as the `cni.rs` metrics tests — the thread-local recorder must survive `.await` points). Wraps a trivial stand-in `tower_service::Service` (not a real tonic handler — `MetricsLayer`/`MetricsService` only depend on the `Service`/`Layer` traits) with `MetricsLayer`, sends a synthetic `http::Request` with URI `/runtime.v1.RuntimeService/RunPodSandbox`, and asserts: the wrapped service's response passes through unchanged, `pelagos_cri_grpc_requests_total{method="RunPodSandbox"}` incremented by 1, and `pelagos_cri_grpc_request_duration_seconds{method="RunPodSandbox"}` recorded exactly one sample. **Why it matters:** this is the only test that exercises the actual `Service` plumbing (the `poll_ready`/`call`/inner-service-swap pattern) end-to-end rather than just the label-mapping function — a bug in the `mem::replace` bookkeeping or the boxed future could silently drop metrics (or the response) without either of the label-mapping unit tests catching it (#498).

## `pelagos-cri::cni::tests::test_cni_metrics_recorded_for_add_and_del`
**Does not require root** (`pelagos-cri` unit test, `cargo test -p pelagos-cri --bin pelagos-cri`). Installs a `metrics_util::debugging::DebuggingRecorder` as the thread-local default recorder (`metrics::set_default_local_recorder`) on a `current_thread` tokio runtime — deliberately `current_thread`, not multi-thread, so the recorder stays active across `run_bounded`'s `.await` points (a multi-thread runtime could resume `run_bounded` after `spawn_blocking(f).await` on a different OS thread, missing a thread-local recorder entirely). Calls `start_semaphore_gauge_sampler()`, then one `run_bounded("add", ...)` and one `run_bounded("del", ...)`. Snapshots the recorder and asserts: `pelagos_cri_cni_semaphore_permits_max` gauge equals `MAX_CONCURRENT_CNI_CALLS` (set synchronously, no need to wait for the 2s sampler tick), and `pelagos_cri_cni_call_duration_seconds` has exactly one histogram sample for each of `op="add"` and `op="del"`. **Why it matters:** this is the regression guard for #499 (CNI invocation metrics, sub-issue of #496) — if the histogram/gauge instrumentation added around the #494 semaphore ever gets silently dropped (e.g. during a future refactor of `run_bounded`), the CNI call metrics that #500 (fail-fast semaphore behavior change) needs real saturation data from would go quietly empty, and nobody would notice until asking "why is this Grafana panel blank" long after the code changed (#499).

## `pelagos-cri::cni::tests::test_run_bounded_limits_concurrency_and_fails_fast_over_capacity`
**Does not require root** (`pelagos-cri` unit test, `cargo test -p pelagos-cri --bin pelagos-cri`). `#[serial(cni_semaphore)]` — see below. Successor to the pre-#500 `test_run_bounded_limits_concurrency`: spawns `MAX_CONCURRENT_CNI_CALLS * 3` (24) concurrent calls through `run_bounded`, each holding its permit for 100ms (long enough that all 24 reach their `try_acquire()` call before any release, even on a loaded CI runner — this is what makes the exact-count assertions below non-flaky). Asserts observed peak concurrency never exceeds 8 (unchanged property from #494), **and** that exactly `MAX_CONCURRENT_CNI_CALLS` (8) calls succeed while exactly `MAX_CONCURRENT_CNI_CALLS * 2` (16) are rejected with `RunBoundedError::ResourceExhausted`. **Why it matters:** this is the direct behavioral regression test for #500 — before #500, all 24 would eventually succeed via `acquire().await` queuing; after #500 (`try_acquire()`), calls beyond capacity are rejected immediately instead of queued. If `run_bounded` ever silently reverts to queuing, this test's exact `ok_count == 8` assertion fails because more than 8 would eventually succeed (#500).

## `pelagos-cri::cni::tests::test_cni_add_bounded_fails_fast_with_resource_exhausted_when_saturated`
**Does not require root** (`pelagos-cri` unit test). `#[serial(cni_semaphore)]` and `current_thread` runtime — see below. Installs a `metrics_util::debugging::DebuggingRecorder`, then holds every permit on the real (module-global) CNI semaphore via `try_acquire_many(MAX_CONCURRENT_CNI_CALLS)` so the next call is guaranteed saturated rather than racing a real acquire/release. Calls `cni_add_bounded` with a bogus sandbox/netns/conf (never actually reached — rejected before the closure runs) and asserts the result is `Err(CniAddError::ResourceExhausted)` specifically, not a generic error, and that `pelagos_cri_cni_semaphore_rejected_total{op="add"}` incremented by 1. **Why it matters:** `runtime.rs`'s `RunPodSandbox` handler pattern-matches on `CniAddError` to decide between gRPC `RESOURCE_EXHAUSTED` (kubelet retries via its normal backoff) and `INTERNAL` (a real plugin failure) — if that distinction is ever lost (e.g. `CniAddError` collapsed back to a plain `String`), saturation would look identical to a genuine CNI failure to kubelet, defeating the point of #500's fail-fast change (#500).

### Why `#[serial(cni_semaphore)]` on the four `pelagos-cri::cni` tests above (plus `test_cni_metrics_recorded_for_add_and_del`)
`cni_semaphore()` is a single process-global `OnceLock<Semaphore>`, and `cargo test` runs tests within a crate concurrently by default. Before #500 this was harmless — `acquire().await` just queued across tests sharing the semaphore. After #500 (`try_acquire()`), a test that deliberately saturates the semaphore (or generates enough concurrent load to do so incidentally) can cause an *unrelated* concurrently-running test's `run_bounded` call to spuriously fail with `ResourceExhausted` — this was caught directly: adding the saturation test above without `#[serial]` made `test_run_bounded_does_not_block_async_worker` flake in exactly this way in the same PR. `#[serial(cni_semaphore)]` forces all five semaphore-touching tests in this module to run one at a time relative to each other (but still concurrently with unrelated tests elsewhere in the crate).

## `pelagos-cri::cni::tests::test_run_bounded_does_not_block_async_worker`
**Does not require root** (`pelagos-cri` unit test, single-worker-thread tokio runtime). Spawns 4 concurrent `run_bounded` calls whose closures do a blocking `std::thread::sleep(200ms)`, then — while those are in flight — awaits a plain `tokio::time::sleep(10ms)` on the runtime's sole async worker thread and asserts it completes within 150ms. **Why it matters:** this is the direct regression test for #494's root cause — `cni_add`/`cni_del`/`create_netns`/`delete_netns` were `std::process::Command` calls made synchronously inline inside async gRPC handlers (`RunPodSandbox`/`StopPodSandbox`), each physically pinning a tokio worker thread for up to 30s while the CNI plugin retried a not-yet-existing socket. With ~12 worker threads (one per CPU on the affected node) and ~40 pods reconciling simultaneously after a reboot, every worker was pinned at once, so *unrelated* RPCs — including `ListContainers`/`Version` and the CRI calls needed to start Cilium's own init containers — timed out with `DeadlineExceeded`, creating a self-sustaining stall (Cilium can't start because everything is blocked waiting on Cilium's socket, and that blocked traffic starves the very calls needed to start Cilium). The fix moves the blocking I/O onto `tokio::task::spawn_blocking`'s separate blocking pool; if `run_bounded` ever regresses to calling its closure inline, this test's own async sleep stalls behind the 200ms blocking closures and blows past the 150ms threshold, failing loudly (verified: reverting the fix locally made this test fail with an observed ~800ms delay).

## `issue_520_cgroup_move_errno::test_ms_move_errno_survives_subsequent_rmdir`
**Requires root** (`mount(MS_MOVE)` needs `CAP_SYS_ADMIN`; no container/namespace spawn needed — the bug is a pure syscall-ordering issue reproduced directly). Forces `mount(MS_MOVE)` to fail deterministically (source is a plain directory, not a mountpoint → `EINVAL`) and captures the errno immediately afterward, mirroring the fixed code in `src/container.rs`. Then runs `rmdir()` on the same (deliberately non-empty) source directory, which fails with a *different* errno (`ENOTEMPTY`), and asserts the originally captured `EINVAL` was not overwritten. **Why it matters:** the cgroup-staging pre_exec closure (`src/container.rs`, both the `pelagos run` and OCI `create` spawn sites, `#455`) calls `mount(cg_src, cg_tgt, ..., MS_MOVE, ...)` and then *unconditionally* runs `rmdir(cg_src)` before reading `io::Error::last_os_error()` to build the error for a failed move. Since `rmdir()` clobbers `errno`, any genuine `MS_MOVE` failure was reported with the wrong (rmdir's) errno instead of the real one. This surfaced as issue #520: `pelagos run --device ...` on a DGX Spark host (CDI GPU passthrough, #512/#513/#516) failed 100% of the time with a misleading `EOVERFLOW` ("Value too large for data type") that vanished under `strace -f` — consistent with a masked, timing-sensitive `MS_MOVE` race whose real errno was never surfaced. The fix reads `last_os_error()` immediately after `MS_MOVE`, before `rmdir()` runs, at both call sites.

## `rootless_idmap::test_rootless_bind_mount_resolves_symlink_in_non_top_layer`
**Requires non-root + the shared alpine-rootfs** (no `fuse-overlayfs` needed — exercises the native-overlay pre-seed path directly). Regression test for #522's real root cause (distinct from the single-uid-map case above, which #524 already fixed). Builds a synthetic two-layer image where the bottom layer has `/lib` as a symlink to `usr/lib` (with real content already in `usr/lib`) and the top layer has no `/lib` entry at all. Binds a file to `/lib/newfile` — forcing resolution through the symlink — under an explicit single-UID rootless map, and asserts: the container succeeds, `/lib` still resolves as a symlink to `usr/lib` afterward, and the bound file landed at the symlink-resolved location (`/usr/lib/newfile`). **Why it matters:** to avoid the rootless native-overlay+userxattr copy-up `EOVERFLOW` (same failure class as the single-uid-map test above, but occurring even with a *full* subordinate uid/gid range — confirmed on real DGX Spark hardware), pelagos pre-seeds bind-mount targets directly into the overlay upper dir before the overlay is even mounted, sidestepping the kernel copy-up path entirely. The first attempt at this fix resolved path symlinks against only the *topmost* overlay layer — silently mis-resolving any symlink that exists only in a deeper layer (like `/lib` -> `usr/lib`, present only in the base OS layer of real multi-layer images) as a literal path component. That created a real directory at the wrong location in the upper dir, shadowing the true symlink entirely and making `/lib` vanish from the merged view — which broke the dynamic linker for every exec in the container (confirmed via `execve()` returning `ENOENT` on real hardware, traced down to `/lib/ld-linux-*.so.1` being unreachable). The fix (`resolve_mount_target_in_layers()`) checks all lower layers in priority order, matching overlayfs's own merge semantics.
