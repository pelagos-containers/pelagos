...
///
/// Dispatches to the builtin `pelagos-dns` or dnsmasq based on `active_backend()`.
...
pub fn dns_add_entry(
    ...
    // Use file locking to prevent races between concurrent `pelagos run` invocations.
...
// ---------------------------------------------------------------------------
// Builtin backend (pelagos-dns)
// ---------------------------------------------------------------------------

/// Start the builtin `pelagos-dns` daemon via double-fork.
fn ensure_builtin_daemon() -> io::Result<()> {
    ...
    // Find the pelagos-dns binary next to the current executable.
...
/// Find the `pelagos-dns` binary. Looks next to the current executable first,
/// then falls back to PATH.
fn find_dns_binary() -> io::Result<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        // Try next to current executable (e.g. target/debug/pelagos-dns).
