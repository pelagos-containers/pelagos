    ...
    */
    ...
    fn convert_poll_message(
        ...
        if let Some(authority) = pseudo.authority {
            ...
            // Some clients (e.g. k3s/kubelet) send a URL-encoded Unix socket path as the
            // :authority (e.g. %2Frun%2Fpelagos%2Fcri.sock). The http crate rejects this as
            // an invalid authority. Rather than RST_STREAM the request, fall back to
            // "localhost" so the gRPC call can proceed normally.
            parts.authority = Some(maybe_authority.unwrap_or_else(|why| {
                tracing::debug!(
                    "malformed :authority ({:?}): {} — substituting localhost",
                    authority,
                    why,
                );
